...
**问题修复：**
- 🐛 **扫码登录**：确认环节失败/无真实 uid 时不再降级成 `uid=0` 的「临时用户」假成功，改为如实返回失败提示重新扫码——修复假登录导致后续网盘 API 全部 `errno=-6`（PR #58）
- 🐛 **系统设置**：左侧锚点导航点击不再把顶部「保存设置」栏顶出可视区（只滚动内容容器，不再连带滚动外层）（PR #59）
- 🐛 **分享同步·手动触发**：触发前做前置校验并返回真实 `run_id`，不再「假成功」；修复为「立即返回 run_id」预建 run 引入的孤儿 run（早退路径未收尾导致永久 `running`）与 `resume_link_invalid` 的状态不一致（基于 @hamr-hub PR #111 移植 + 修复，详见 PR #60）
- 🐛 **分享同步·调度器**：`on_tick` 改为返回 `Result` 不再静默吞错，失败落 `warn` 日志便于排查（PR #111 / #60）
- 🐛 **分享同步·下载等待**：等待逻辑改为「空闲超时 + 硬上限」双闸，文件夹聚合速度改用「活跃下载状态」判断，修复进行中速度恒显示 0（PR #111 / #60）
- 🐛 **分享同步·历史数据**：启动期对 `owner_uid=0` 历史脏数据按活跃账号迁移归属（PR #111 / #60）

**新功能：**
- ✨ **分享同步·运行明细分页**：新增 `GET /share-sync/runs/:id/items` 分页接口（`count` + 分页查询），前端运行详情弹窗接入分页，避免大 run 明细被截断到 100 条（PR #111 / #60）
- ✨ **分享同步·下载停滞自动重试**：检测到下载长时间无进展时自动 `pause→冷却→resume` 重启（限次数 + 冷却，可经环境变量调参）（PR #111 / #60）

**工程/CI：**
- 🔧 统一 Rust 工具链版本为 1.87（本地 / Docker / CI 一致）（PR #56）
- 🔧 增加 Conventional Commits 校验（PR 标题 + 逐 commit）（PR #57）

> 致谢：分享同步相关后端修复与增强移植自 @hamr-hub 的 PR #111（保留原作者署名），并在下游修复了为「立即返回 run_id」引入的孤儿 run 与恢复误报、回退了与本次修复无关的前端三栏布局改版。
