...
- 🐛 **分享同步·手动触发**：触发前做前置校验并返回真实 `run_id`，不再「假成功」；修复为「立即返回 run_id」预建 run 引入的孤儿 run（早退路径未收尾导致永久 `running`）与 `resume_link_invalid` 的状态不一致
- 🐛 **分享同步·调度器**：`on_tick` 改为返回 `Result` 不再静默吞错，失败落 `warn` 日志便于排查
- 🐛 **分享同步·下载等待**：等待逻辑改为「空闲超时 + 硬上限」双闸，文件夹聚合速度改用「活跃下载状态」判断，修复进行中速度恒显示 0
- 🐛 **分享同步·历史数据**：启动期对 `owner_uid=0` 历史脏数据按活跃账号迁移归属

**新功能：**
- ✨ **分享同步·运行明细分页**：新增 `GET /share-sync/runs/:id/items` 分页接口（`count` + 分页查询），前端运行详情弹窗接入分页，避免大 run 明细被截断到 100 条
- ✨ **分享同步·下载停滞自动重试**：检测到下载长时间无进展时自动 `pause→冷却→resume` 重启（限次数 + 冷却，可经环境变量调参）
